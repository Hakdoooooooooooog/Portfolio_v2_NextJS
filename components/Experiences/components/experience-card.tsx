...
const ExperienceBullets = ({ bullets }: { bullets: string[] }) => {
  return (
    <ul className="list-disc list-outside pl-5 flex flex-col gap-2 text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
  ...
  return (
    <ul className="flex justify-start flex-wrap gap-y-2 gap-x-4 text-sm">
      {skills.map((skill, index) => (
        <li
          key={index}
          className="p-2 bg-blue-300 dark:bg-blue-700/50 text-blue-900 dark:text-gray-300 text-sm rounded-lg shadow-sm hover:bg-blue-300 dark:hover:bg-gray-700 transition-colors duration-200"
        >
          {skill}
        </li>
      ))}
